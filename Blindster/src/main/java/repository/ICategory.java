...
public interface ICategory extends Repository<Category, Long>{
    ...
    @Query(value = "SELECT * FROM category c WHERE c.parentId=:parentId OR c.id=:id ORDER BY c.position", nativeQuery = true)   
    List<Category> findByParentId(long parentId, long id);
