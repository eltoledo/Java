...
public class ProductController {
    ...
    public ResponseEntity<ProductDTO> add(@Validated @RequestBody ProductDTO productDTO){
        ...
        productDTO = productMapper.toDTO(productService.add(p));
    ...
    @GetMapping(path = {"/{id}"})
    public ResponseEntity<ProductDTO> toProductById(@PathVariable(value = "id") long id){
        return new ResponseEntity<>(productMapper.toDTO(productService.toProductById(id)), HttpStatus.OK);
    }
}
